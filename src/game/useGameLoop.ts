...
// Autosave, an upgrade and page teardown can all ask for a save at once. Fetch
// responses are not ordered: without a queue an older snapshot may arrive last,
// replace a newer balance and make the following honest save look impossible.
// Snapshot the payload at invocation time, then send requests in that same order.
let _saveQueue: Promise<void> = Promise.resolve()

function saveToServer(gs: GameState, joints: number, sats: number, totalJointsEarned: number, _activeBoosts: ActiveBoost[] = [], _speedLevel = 0, _jointsRev = 0): Promise<void> {
  const auth = JSON.parse(localStorage.getItem('jf_auth') || '{}')
  ...
  const queued = _saveQueue.then(send, send)
  _saveQueue = queued.then(() => undefined, () => undefined)
  return queued
}
...
export function useGameLoop(
  ...
  /**
   * Write now and wait for it, instead of somewhere in the next thirty seconds.
   *
   * The server learns the round was won from a save — that is where noteProgress
   * stamps it. The card asked the server the moment the client crossed the line,
   * got "not yet", and the fireworks only turned up after a refresh, once a save
  ...
  return {
    ...
    actions: {
      ...
      unlockPlantation, buyBoost, buySpeed, claimBoost, saveNow,
    },
    boostGrants,
  }
}
