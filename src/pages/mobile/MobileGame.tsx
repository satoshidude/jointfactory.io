...
function SatsballBanner() {
  return (
    <a
      className="satsball-banner"
      href="https://satsball.jointfactory.io"
      target="_blank"
      rel="noopener noreferrer"
      aria-labelledby="satsball-title"
    >
      <div className="satsball-banner-copy">
        <span className="satsball-kicker"><CircleDot size={11} /> Side game · 10 sats</span>
        <h2 id="satsball-title">Satsball</h2>
        <p>Drop three balls. Match a column. Win sats.</p>
      </div>
      <div className="satsball-open" aria-hidden="true">
        <span>Play</span>
...
export default function MobileGame() {
  ...
  const gate = ticketGate(state, state.roundsCompleted, state.satsIntoPot)
  ...
  // The objective must quote the next ticket, not always ticket #1. A player
  // who already holds one faces the steeper second step of the draw's curve.
  // Until the lottery card has loaded the held count, suppress the claim.
  const nextTicketCost = gd.ticketsHeld === undefined ? Infinity : ticketPrice(gd.ticketsHeld, rate)
  const nextSpeedCost = speedCost(state.speedLevel, rate)
  const objective = nextObjective(state, auth.isLoggedIn, state.joints >= nextTicketCost)
