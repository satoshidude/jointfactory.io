 ...
 */
export default function RoundCard({ totalEarned, isLoggedIn, onReset, embedded = false }: {
  totalEarned: number
  isLoggedIn: boolean
  onReset?: () => void
  ...
  // The server decides when the target fell; ask again once the client thinks it
  // has, so the reset button appears without a reload.
  useEffect(() => {
    if (!isLoggedIn || !info || info.reached_target_at) return
    if (totalEarned < ROUND_TARGET) return
    load()
  }, [totalEarned, info, isLoggedIn, load])
