...
export default function LoginModal({ onClose }: Props) {
  ...
  async function doAuth(signedEvent: unknown) {
    ...
    try {
      ...
      onClose();
      // Always land on the chain. Logging in from the info page or the profile
      // left the player there, looking at a page that says nothing about the
      // account they just opened. Grow mounts the loop, which loads the stored
      // state — so this is also what puts the real save on screen.
      navigate('/');
