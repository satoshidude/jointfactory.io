...
export async function createInvoice(npub, packId) {
  ...
  // A row keyed to something that is not an account can never be credited to
  // one, and it sits in the table as a payment that will never resolve. The
  // route always passes the right thing; a script called by hand does not, and
  // one did — a pending 50-sat invoice keyed to the string "50.0".
  if (typeof npub !== 'string' || !/^[0-9a-f]{64}$/.test(npub)) {
    throw new Error('createInvoice needs a 64-char hex npub, got: ' + JSON.stringify(npub));
  }
