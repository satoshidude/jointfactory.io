...
/** Material already inside the chain, wherever it currently waits or moves. */
function chainStock(state) {
  return Math.max(0, state?.cannabis || 0)
    + Math.max(0, state?.courier?.carrying || 0)
    + Math.max(0, state?.cannabisAtFactory || 0)
    + Math.max(0, state?.fabrik?._currentCharge || 0);
}
...
const _saveStateTx = db.transaction((npub, payload) => {
  ...
  if (staleBalance) {
    ...
    correctionReason = 'stale_revision';
  } else if (existing) {
    // Since the last *save*, not since the last sign-in: logging in used to set
    ...
    // A fast chain produces in discrete loads, not in the smooth average above.
    // A courier load or an almost-finished factory batch can therefore complete
    // far more than a few seconds of steady-state throughput between two action
    // saves. Account for it by conservation instead of another rate estimate:
    // cannabis in the field, courier, factory store and current charge is the
    // same material moving through the chain. Only turning it into joints makes
    // that total fall. Comparing both snapshots also prevents the same stored
    // load from being claimed again on every save.
    const storedStock = chainStock(stored);
    const incomingStock = chainStock(gameState);
    const fromStock = Math.max(0, storedStock - incomingStock);
    ...
    // Affordability is checked against the stored chain below. A client must
    // not be able to finance an upgrade with the higher allowance that the
    // upgrade itself creates. Once that purchase is proven affordable, though,
    // its production is real: action saves land one second after the click, and
    // the upgraded bottleneck may already have completed work in that window.
    // Measuring all of it against the old rate caused 75 of the last 135 live
    // clamps, usually directly beside an upgrade event.
    const baselineAllowance = (rate * elapsed + fromStock) * 1.5 + 1000;
    let allowance = baselineAllowance;
    guard = {
      elapsed: Math.round(elapsed),
      rate: Math.round(rate), incoming_rate: Math.round(rate),
      stock: Math.round(fromStock),
      stored_stock: Math.round(storedStock),
      incoming_stock: Math.round(incomingStock),
      carrying: Math.round(Math.max(0, stored.courier?.carrying || 0)),
      current_charge: Math.round(Math.max(0, stored.fabrik?._currentCharge || 0)),
    ...
    const impossible = regressed || jump > MAX_LEVEL_STEP || overCap || spent > existing.joints + baselineAllowance;
    if (impossible) {
      ...
      // for the next one.
      keepStoredState = true;
      correctionReason = regressed ? 'state_regressed'
        : overCap ? 'level_over_cap'
        : jump > MAX_LEVEL_STEP ? 'level_jump'
        : 'unaffordable_upgrade';
    } else {
      // The state and its cost have passed every structural and affordability
      // check. From here on the new chain is a paid-for production baseline,
      // not attacker-controlled headroom. Use the faster of the two rates: an
      // upgrade may happen anywhere inside the save window, and choosing the
      // maximum is deliberately generous by at most one window while avoiding
      // confiscation of a discrete batch completed immediately after purchase.
      let incomingRate = rate;
      try { incomingRate = throughput(gameState, opts).jointsPerSec; } catch { /* keep stored rate */ }
      incomingRate = Math.max(rate, Number.isFinite(incomingRate) ? incomingRate : rate);
      allowance = (incomingRate * elapsed + fromStock) * 1.5 + 1000;
      guard.incoming_rate = Math.round(incomingRate);
      guard.allowance = Math.round(allowance);
      totalCeiling = Math.min(ROUND_TARGET, (existing.total_joints_earned || 0) + allowance);
      ...
      if (plausible > ceiling) {
        ...
        correctionReason = bought?.cost > 0 ? 'production_after_upgrade'
          : fromStock > 0 ? 'stock_conversion'
          : 'production_ceiling';
      }
    }
  }
  ...
  if (corrected) logEvent(npub, 'clamp', reported - plausible, {
    ...
    reason: correctionReason || 'unknown',
