...
import { rollupDay, recentStats, METRICS_VERSION } from './metrics.js';
...
fastify.get('/api/health/metrics', { preHandler: requireOwner }, async (req) => {
  ...
  return {
    ...
    definitions: {
      active: 'distinct non-bot players starting a genuine session after 30 minutes away',
      wau: 'distinct active players in Berlin calendar days D-6 through D',
      mau: 'distinct active players in Berlin calendar days D-29 through D',
      retention: 'exact-day return of genuine signup cohorts; backfills excluded',
    },
    solvency: solvency(),
    days: recentStats(days),
  };
});
